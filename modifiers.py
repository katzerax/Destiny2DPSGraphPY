import math
    ...
    #hi roxy your code had compiler errors so I fixed those so VSC would stop yelling at me in the future program better
...
def FocusedFury(FFActive,shots_fired,magazine_capacity,damage_per_shot,time_elapsed): #checks for whether it is active, then for whether it should activate, then the activation requirements before setting itself active, followed by a check on how to turn it back
    if FFActive == 0:
        if shots_fired == math.ceil(magazine_capacity/2): #activates when shots
            shot_dmg_output = shot_dmg_output * 1.2
            FFActive = 1
        else:
            #blah blah fix your formatting dumbass - K
            print("REMOVE ME LOL")
    else:
        if time_elapsed % 10 == 0: #this time check logic needs to be looked at, probably use another variable to track when the timer would be up for the buff
            # K here, you did not pass time_elapsed into this function and im not sure that you even intend to! but I decided to pass it here so that it would stop yelling at me. lmao;
            shot_dmg_output = damage_per_shot
            FFActive = 0
        else:
            #0 = 0 #hey cool statement that not only doesnt do anything but breaks python :clap:
            print("REMOVE ME LOL")
...
def FiringLine(shot_dmg_output): #flat bonus 20%, for all cases, permanently, dream
    shot_dmg_output = shot_dmg_output * 1.2
